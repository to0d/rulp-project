...
std::string get_project_dir() {
  const char* projectDir = getenv("PROJECT_HOME");  
  return FS_TO_VALID_PATH(std::string(projectDir));
}
...
void run_rulp_script(StrConstRef inputScriptPath, lfc::function<void(IRInterpreter*)> init_func){
  {
    ...
    std::string home_dir = get_project_dir();
    info = STR_REPLACE_ALL(info, home_dir, "");
